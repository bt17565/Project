<div>
                    ...
                    @foreach ($this->user->tokens->sortBy('name') as $token)
                        <div class="d-flex justify-content-between">
                            ...
                            <div class="d-flex">
                                ...
                                @if (Laravel\Jetstream\Jetstream::hasPermissions())
                                    <button class="btn btn-link text-secondary text-decoration-none running_case" wire:click="manageApiTokenPermissions({{ $token->id }})">
                                        <span>{{ __('Permissions') }}</span>
                                    </button>
                                @endif

                                <button class="btn btn-link text-danger text-decoration-none running_case" wire:click="confirmApiTokenDeletion({{ $token->id }})">
                                    {{ __('Delete') }}
                                </button>
                            </div>
                        </div>
    ...
    <x-jet-dialog-modal wire:model="managingApiTokenPermissions">
        ...
        <x-slot name="footer">
            <x-jet-secondary-button class="mr-3" wire:click="$set('managingApiTokenPermissions', false)" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-jet-secondary-button>
    ...
    <x-jet-confirmation-modal wire:model="confirmingApiTokenDeletion">
        ...
        <x-slot name="footer">
            <x-jet-secondary-button class="mr-3" wire:click="$toggle('confirmingApiTokenDeletion')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-jet-secondary-button>
